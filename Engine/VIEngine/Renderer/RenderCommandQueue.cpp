...
namespace VIEngine {
	RenderCommandQueue::RenderCommandQueue(): mRenderCommandCallbackQueue(), mMemmoryManager() {
	...
	void RenderCommandQueue::Enqueue(const RenderCallback& renderCallback) {
		RenderCommandCallback* renderCommandCallback = mMemmoryManager.NewPerFrame<RenderCommandCallback>(renderCallback);
